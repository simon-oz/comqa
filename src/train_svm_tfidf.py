...
from sklearn.linear_model import LogisticRegression  # Not used here, but kept for SafeOneVsRest compatibility if needed.
...
# SVM hyperparameters.
# Keep these the same as your current embedding-based SVM if desired.
SVM_C = 3.0
...
def setup_output_logging():
    ...
    log_path = LOG_DIR / f"train_svm_tfidf_{timestamp}.log"
...
def build_tfidf_features(train_texts, dev_texts, test_texts):
    ...
    Returns sparse matrices compatible with LinearSVC.
    """
...
def main():
    ...
    print("=" * 90)
    print("Linear SVM multi-label classifier with TF-IDF features")
    print("=" * 90)
    ...
    _, thresholds = evaluate_predictions(
        ...
        dev_scores,
        threshold=None,
        dataset_name="Linear SVM TF-IDF dev",
    )
    ...
    if test_has_labels:
        test_preds, _ = evaluate_predictions(
            ...
            dataset_name="Linear SVM TF-IDF test",
        )
    else:
    ...
    model_path = ARTIFACT_DIR / "svm_tfidf_ovr.joblib"
    threshold_path = ARTIFACT_DIR / "svm_tfidf_thresholds.npy"
    vectorizer_path = ARTIFACT_DIR / "svm_tfidf_vectorizer.joblib"
    ...
    save_predictions(
        test_df,
        test_preds,
        PREDICTION_DIR / "test_svm_tfidf_predictions.csv",
    ...
    # Save timing
    # --------------------------------------------------------
    timing_path = ARTIFACT_DIR / "svm_tfidf_timing.json"
